...
export default function AddonManagerHandler(mainWindow) {
    ...
    ipcMain.handle('update-addons', async (_) => {
        ...
        const isWifiAvailable = await new Promise((resolve, _) => {
        ...
        for (const addon of addons) {
            ...
            new Promise((resolve, reject) => {
                exec(`git pull`, { cwd: addonPath, env: { ...process.env, LANG: 'en_US.UTF-8' } }, (err, stdout, _) => {
                    if (err) {
                        sendNotification({
...
//# sourceMappingURL=handler.addon.js.map