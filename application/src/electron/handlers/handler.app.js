...
export async function addToDesktop() {
    ...
    let execPath = path.resolve(appDirpath, fs.readdirSync(appDirpath).find((file) => file.endsWith('.AppImage')) ||
        './OpenGameInstaller.AppImage') ?? './OpenGameInstaller.AppImage';
    try {
        ...
        // Get executable path
        const setupAppImagePath = path.resolve(path.resolve(appDirpath, '..'), 'OpenGameInstaller-Setup.AppImage');
        ...
        if (app.isPackaged) {
            iconPathForFavicon = path.join(app.getPath('exe'), '..', 'opengameinstaller-gui.png');
        }
        else {
            // In development, use the public folder
            iconPathForFavicon = path.join(__dirname, '..', '..', 'public', 'favicon.png');
        }
        // dump the icon path into the appdirpath/favicon.png
        if (iconPathForFavicon) {
            createReadStream(iconPathForFavicon).pipe(createWriteStream(path.join(appDirpath, 'favicon.png')));
        }
...
export default function handler(mainWindow) {
    ...
    // Addon helpers
    ipcMain.handle('app:get-addon-path', async (_, addonID) => {
    ...
    ipcMain.handle('app:get-addon-icon', async (_, addonID) => {
        let client = clients.get(addonID);
        if (!client || !client.filePath) {
            return null;
        }
        // read the addon.json file to get the icon path
        const addonJson = JSON.parse(fs.readFileSync(join(client.filePath, 'addon.json'), 'utf-8'));
        if (!addonJson.icon) {
            return null;
        }
        const iconPath = join(client.filePath, addonJson.icon);
        if (!fs.existsSync(iconPath)) {
            console.error('No icon path found for addon (does not exist): ' +
                addonID +
                ' at path: ' +
                iconPath);
            return null;
    ...
    ipcMain.handle('app:get-local-image', async (_, path) => {
...
//# sourceMappingURL=handler.app.js.map