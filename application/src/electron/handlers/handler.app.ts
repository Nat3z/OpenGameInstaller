...
export const addToDesktopEffect = (): Effect.Effect<
  { success: true; path: string } | { success: false; error: string },
  FileSystemError
> => {
  if (process.platform === 'win32') {
    return Effect.succeed({ success: false, error: 'This feature is only available on Linux' });
  }
  return Effect.gen(function* () {
    let appDirPath = isDev() ? `${app.getAppPath()}/../` : path.dirname(process.execPath);
    if (process.platform === 'linux') appDirPath = './';
    let execPath = path.resolve(
      appDirPath,
      fs.readdirSync(appDirPath).find((file) => file.endsWith('.AppImage')) ?? './OpenGameInstaller.AppImage'
    );
    ...
    yield* Effect.tryPromise({
      ...
      catch: (cause) => new FileSystemError({ message: formatError(cause), path: desktopDir, cause }),
    });
    const setupPath = path.resolve(path.resolve(appDirPath, '..'), 'OpenGameInstaller-Setup.AppImage');
    ...
    yield* Effect.tryPromise({
      ...
      catch: (cause) => new FileSystemError({ message: formatError(cause), path: sourceIcon, cause }),
    });
    ...
    yield* Effect.tryPromise({
      try: () => fsAsync.writeFile(desktopFilePath, desktopContent, { mode: 0o755 }),
      catch: (cause) => new FileSystemError({ message: formatError(cause), path: desktopFilePath, cause }),
    });
    return { success: true as const, path: desktopFilePath };
  });
};

/** Promise compatibility boundary used by migrations and legacy callers. */
export const addToDesktop = (): Promise<{ success: boolean; path?: string; error?: string }> =>
  Effect.runPromise(addToDesktopEffect().pipe(
    Effect.catchAll((error) => Effect.succeed({ success: false as const, error: error.message }))
  ));

const axiosRequest = (options: AxiosRequestConfig): Effect.Effect<
...
> => Effect.tryPromise({
  try: async () => {
    if (options.data && options.headers &&
      ...
      for (const [key, value] of Object.entries(options.data)) formData.append(key, value as string);
...
export default function handler(mainWindow: Electron.BrowserWindow): void {
  const syncHandlers: Array<[string, () => void]> = [
    ...
    ['app:show-window', () => { if (mainWindow && !mainWindow.isDestroyed()) { mainWindow.show(); mainWindow.focus(); } }],
  ...
  ipcMain.handle('app:axios', (_, options: AxiosRequestConfig) =>
    Effect.runPromise(axiosRequest(options).pipe(
      Effect.catchAll((error) => Effect.succeed({ data: error.message, status: error.statusCode, success: false }))
    ))
  );
  ipcMain.handle('app:get-os', () => runBoundary(Effect.succeed(process.platform)));
  ipcMain.handle('app:is-steam-deck', () => runBoundary(
    Effect.try({
      try: () => process.platform === 'linux' && getCurrentUsername()?.toLowerCase() === 'deck',
      catch: (cause) => new PlatformError({ message: formatError(cause), platform: process.platform }),
    })
  ));
  ipcMain.handle('app:screen-input', (_, data) => runBoundary(Effect.sync(() => {
    currentScreens.set(data.id, data.data);
    screenInputCallbacks.get(data.id)?.(data.data);
    screenInputCallbacks.delete(data.id);
  })));
  ipcMain.handle('app:is-online', () => runBoundary(Effect.sync(() => getEffectiveOnlineState().effectiveOnline)));

  ipcMain.handle('app:get-addon-path', (_, addonID: string) => runBoundary(
    Effect.sync(() => addonServer.getClient(addonID)?.filePath ?? null)
  ));
  ipcMain.handle('app:get-addon-icon', (_, addonID: string) => runBoundary(
    Effect.try({
      try: () => {
        const client = addonServer.getClient(addonID);
        if (!client?.filePath) return null;
        const addonJson = JSON.parse(fs.readFileSync(join(client.filePath, 'addon.json'), 'utf-8')) as { icon?: string };
  ...
  ipcMain.handle('app:get-local-image', (_, requestPath: string) => runBoundary(
    Effect.gen(function* () {
      if (!fs.existsSync(requestPath)) return null;
      const allowedDirs = [
        join(__dirname, 'addons'), join(__dirname, 'public'), join(__dirname, 'config'),
        ...Array.from(addonServer.getConnections()).flatMap((connection) => connection.filePath ? [connection.filePath] : []),
      ].map((directory) => path.resolve(directory));
      const realPath = yield* Effect.try({
        try: () => fs.realpathSync(requestPath),
        catch: (cause) => new FileSystemError({ message: formatError(cause), path: requestPath, cause }),
      });
      const allowed = allowedDirs.some((directory) => {
        const relative = path.relative(directory, realPath);
        return relative === '' || (!relative.startsWith('..') && !path.isAbsolute(relative));
      });
      if (!allowed) return yield* Effect.fail(new FileSystemError({ message: 'Path is outside allowed directories', path: realPath }));
      const ext = path.extname(realPath).slice(1).toLowerCase();
      const mimeType = ({ jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', gif: 'image/gif', webp: 'image/webp', bmp: 'image/bmp', svg: 'image/svg+xml' } as Record<string, string>)[ext] ?? 'image/png';
      const buffer = yield* Effect.tryPromise({
        try: () => fsAsync.readFile(realPath),
        catch: (cause) => new FileSystemError({ message: formatError(cause), path: realPath, cause }),
      });
      return `data:${mimeType};base64,${buffer.toString('base64')}`;
    })
  ));

  ipcMain.handle('app:add-to-desktop', () => runBoundary(addToDesktopEffect()));
