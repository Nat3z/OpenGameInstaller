...
class Download {
    ...
    chunks = [];
    parallelTotalSize = 0;
    currentJobPath = ''; // Track current job path for chunk file cleanup
    // Multi-part parallel download state
    useParallelParts = false;
    parts = [];
    multiPartTotalBytes = 0;
    multiPartStartTime = 0;
    constructor(mainWindow, jobs, startPart = 1) {
     ...
     */
    async runParallelParts() {
        this.useParallelParts = true;
        // Only re-initialize parts if they don't exist (first run, not resume)
        if (this.parts.length === 0) {
            this.parts = [];
        }
        console.log(`[direct] Starting parallel multi-part download with ${this.totalParts} parts`);
        ...
        for (let i = 0; i < this.totalParts; i++) {
            ...
            if (fs.existsSync(job.path)) {
                ...
                console.log(`[direct] Part ${i + 1} file exists with ${downloadedBytes} bytes`);
                ...
                try {
                    ...
                    if (parallelInfo.fileSize > 0) {
                        ...
                        if (parallelInfo.useParallel) {
                            ...
                            const allChunksExist = Array.from({ length: effectiveChunkCount }, (_, idx) => {
                    ...
                    console.log(`[direct] Could not verify completion for part ${i + 1}:`, error);
                }
            }
            this.parts.push({
                index: i,
        ...
        const pendingParts = () => this.parts.filter((p) => p.status === 'pending' || p.status === 'downloading');
        const activeParts = () => this.parts.filter((p) => p.status === 'downloading');
        ...
        while (pendingParts().length > 0 || activeParts().length > 0) {
            ...
            if (partsToStart.length > 0) {
                console.log(`[direct] Starting ${partsToStart.length} parts (${activeParts().length} active, ${completedParts().length} completed)`);
            ...
            if (this.parts.every((p) => p.status === 'completed' || p.status === 'failed')) {
                break;
            }
        }
     ...
     */
    async downloadPartWithState(part, retries = 5) {
        const job = part.job;
        let lastError;
        ...
        this.updateMultiPartTotalBytes();
        if (parallelInfo.useParallel) {
            console.log(`[direct] Part ${part.index + 1}: Using chunk parallelization (${(parallelInfo.fileSize / (1024 * 1024)).toFixed(2)}MB)`);
            part.useChunks = true;
            ...
            for (let i = 0; i < retries; i++) {
                ...
                try {
                    await this.executeParallelDownloadForPart(part, parallelInfo.fileSize);
                    ...
                    lastError = error;
                    console.log(`[direct] Part ${part.index + 1} chunk download attempt ${i} failed:`, lastError);
                    // If 429 error, disable chunk parallelization and retry as standard download
                    if (lastError.message === '429_TOO_MANY_REQUESTS') {
                        console.log(`[direct] Part ${part.index + 1}: 429 detected, disabling chunk parallelization and retrying as standard download`);
                        ...
                        try {
                            ...
                            console.log('[direct] Error cleaning up chunk files:', cleanupError);
                        }
        ...
        for (let i = 0; i < retries; i++) {
            ...
            try {
                ...
                lastError = error;
                console.log(`[direct] Part ${part.index + 1} download attempt ${i} failed:`, lastError);
                if (this.status !== 'downloading')
                    throw lastError;
                await new Promise((resolve) => setTimeout(resolve, 1000 * (i + 1)));
            }
     ...
     */
    async shouldUseParallelDownloadForPart(job) {
        try {
            ...
            const parallelLimitHeader = headResponse.headers['ogi-parallel-limit'] ||
                headResponse.headers['OGI-Parallel-Limit'];
            const parallelLimit = parallelLimitHeader
                ? parseInt(parallelLimitHeader, 10)
                : undefined;
            const useParallel = supportsRange &&
            ...
            console.log('[direct] HEAD request failed for part, falling back to standard:', error);
            return {
                useParallel: false,
                fileSize: 0,
                supportsRange: false,
     ...
     */
    async executeParallelDownloadForPart(part, fileSize) {
        const job = part.job;
        part.chunks = [];
        ...
        const chunkPromises = part.chunks.map((chunk) => this.downloadChunkForPart(part, chunk));
        try {
     ...
     */
    downloadChunkForPart(part, chunk) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                chunk.response = await axios.get(part.job.link, {
                    responseType: 'stream',
                    headers,
                    httpAgent: keepAliveAgent,
                    httpsAgent: keepAliveAgent,
                ...
                if (chunk.response.status === 429) {
                    console.log(`[direct] Part ${part.index + 1} chunk ${chunk.index}: 429 Too Many Requests, disabling chunk parallelization`);
                    reject(new Error('429_TOO_MANY_REQUESTS'));
                    return;
                }
                if (chunk.response.status !== 206) {
                    reject(new Error(`Unexpected status ${chunk.response.status} for range request`));
                    return;
                }
                ...
                chunk.response.data.on('data', (data) => {
                    chunk.currentBytes += data.length;
                    part.downloadedBytes = part.chunks.reduce((sum, c) => sum + c.currentBytes, 0);
                });
            }
            catch (error) {
     ...
     */
    async mergeChunkFilesForPart(part) {
        ...
        const effectiveChunkCount = part.effectiveChunkCount ?? PARALLEL_CHUNK_COUNT;
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                const keepAliveAgent = job.link.startsWith('https')
                    ? new https.Agent({ keepAlive: true })
                    : new http.Agent({ keepAlive: true });
                part.response = await axios.get(job.link, {
                    responseType: 'stream',
                    headers,
                    httpAgent: keepAliveAgent,
                    httpsAgent: keepAliveAgent,
                ...
                part.response.data.on('data', (data) => {
                ...
                if (error instanceof AxiosError) {
                    if (error.response?.status === 416) {
                        ...
                        console.log(`[direct] Part ${part.index + 1}: 429 Too Many Requests, will retry`);
                        reject(error);
                        return;
                    }
                }
     ...
     */
    updateMultiPartTotalBytes() {
        this.multiPartTotalBytes = this.parts.reduce((sum, part) => sum + (part.totalBytes || 0), 0);
    }
    pause() {
        if (this.status !== 'downloading')
            return;
    ...
    async downloadPart(job, retries = 5) {
        let lastError;
            ...
            if (parallelInfo.useParallel) {
                ...
                for (let i = 0; i < retries; i++) {
                    ...
                    try {
                        ...
                        lastError = error;
                        console.log('[direct] Error in parallel download attempt', i, lastError);
                        // If 429 error, disable parallelization and retry as standard download
                        if (lastError.message === '429_TOO_MANY_REQUESTS') {
                            console.log('[direct] 429 detected, disabling parallelization and retrying as standard download');
                            // Clean up any partial chunk files
                            try {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (fs.existsSync(job.path)) {
                    ...
                    this.startByte = 0;
                    console.log('[direct] No existing file found, starting from beginning');
                }
                ...
                const keepAliveAgent = job.link.startsWith('https')
                    ? new https.Agent({ keepAlive: true })
                    : new http.Agent({ keepAlive: true });
                this.response = await axios.get(job.link, {
                    responseType: 'stream',
                    headers,
                    httpAgent: keepAliveAgent,
                    httpsAgent: keepAliveAgent,
                ...
                if (this.startByte > 0 && this.response.status !== 206) {
                    ...
                    await rmAsync(job.path, { force: true });
                    // restart
                    console.log("[direct] Restarting download (doesn't support range requests)");
                ...
                stream.on('finish', () => {
                    this.cleanupPart();
                    console.log('[direct] Stream finished (Downloaded bytes:', `${(this.currentBytes / (1024 * 1024)).toFixed(2)} MB)`);
                    resolve();
                });
    ...
    startProgressTracker() {
        this.progressInterval = setInterval(() => {
            if (this.useParallel) {
                ...
                const totalDownloaded = this.chunks.reduce((sum, chunk) => sum + chunk.currentBytes, 0);
                const elapsedTime = (Date.now() - this.startTime) / 1000;
                const downloadSpeed = elapsedTime > 0 ? totalDownloaded / elapsedTime : 0;
                const progress = this.parallelTotalSize > 0
                ...
                const elapsedTime = (Date.now() - this.startTime) / 1000;
                const downloadSpeed = elapsedTime > 0
                    ? (this.currentBytes - this.startByte) / elapsedTime
                    : 0;
                const progress = this.totalSize > 0 ? this.currentBytes / this.totalSize : 0;
                this.sendProgress({ progress, downloadSpeed });
            }
        }, 500);
    }
     ...
     */
    startMultiPartProgressTracker() {
        this.progressInterval = setInterval(() => {
            // Aggregate progress from all parts
            const totalDownloaded = this.parts.reduce((sum, part) => sum + part.downloadedBytes, 0);
            const elapsedTime = (Date.now() - this.multiPartStartTime) / 1000;
            const downloadSpeed = elapsedTime > 0 ? totalDownloaded / elapsedTime : 0;
            const progress = this.multiPartTotalBytes > 0
            ...
            const downloadingParts = this.parts.filter((p) => p.status === 'downloading' || p.status === 'pending');
            const completedParts = this.parts.filter((p) => p.status === 'completed');
     ...
     */
    async shouldUseParallelDownload(job) {
        try {
            const keepAliveAgent = job.link.startsWith('https')
                ? new https.Agent({ keepAlive: true })
                : new http.Agent({ keepAlive: true });
            ...
            const useParallel = supportsRange &&
                ...
                !(job.headers && job.headers['OGI-Parallel-Limit'] === '1') && // If the link served has a parallel limit of 1, don't use it
                !(parallelLimit === 1); // Also check response header
            console.log(`[direct] Parallel check: size=${(contentLength / (1024 * 1024 * 1024)).toFixed(2)}GB, ` +
            ...
            console.log('[direct] HEAD request failed, falling back to standard download:', error);
            return {
                useParallel: false,
                fileSize: 0,
                supportsRange: false,
     ...
     */
    async executeParallelDownload(job, fileSize) {
        ...
        for (let i = 0; i < effectiveChunkCount; i++) {
            ...
            if (fs.existsSync(chunkPath)) {
                chunkCurrentBytes = fs.statSync(chunkPath).size;
                console.log(`[direct] Chunk ${i} file exists with ${chunkCurrentBytes} bytes`);
            }
            this.chunks.push({
                index: i,
                startByte,
        ...
        this.startTime = Date.now();
        this.startProgressTracker();
        console.log(`[direct] Starting parallel download with ${effectiveChunkCount} chunks ` +
            `(limit: ${parallelInfo.parallelLimit ?? 'none'}), ` +
            `chunk size: ${(chunkSize / (1024 * 1024)).toFixed(2)}MB`);
        // Download all chunks in parallel
        const chunkPromises = this.chunks.map((chunk) => this.downloadChunk(job, chunk));
        try {
            await Promise.all(chunkPromises);
    ...
    downloadChunk(job, chunk) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                console.log(`[direct] Chunk ${chunk.index}: downloading bytes ${actualStartByte}-${chunk.endByte} to ${chunkPath}`);
                chunk.response = await axios.get(job.link, {
                    responseType: 'stream',
                    headers,
                    httpAgent: keepAliveAgent,
                    httpsAgent: keepAliveAgent,
                ...
                if (chunk.response.status === 429) {
                    console.log(`[direct] Chunk ${chunk.index}: 429 Too Many Requests, disabling parallelization`);
                    reject(new Error('429_TOO_MANY_REQUESTS'));
                    return;
                }
                if (chunk.response.status !== 206) {
                    console.log(`[direct] Chunk ${chunk.index}: unexpected status ${chunk.response.status}`);
                    reject(new Error(`Unexpected status ${chunk.response.status} for range request`));
                    return;
                }
                ...
                if (error instanceof AxiosError) {
                    if (error.response?.status === 416) {
                        ...
                        console.log(`[direct] Chunk ${chunk.index}: range not satisfiable, marking complete`);
                        chunk.completed = true;
                        resolve();
                        return;
                    }
     ...
     */
    async deleteChunkFiles(basePath) {
        ...
        for (let i = 0; i < PARALLEL_CHUNK_COUNT; i++) {
            ...
            deletePromises.push(rmAsync(chunkPath, { force: true }).catch((e) => console.error(`Failed to delete chunk file ${chunkPath}`, e)));
        }
    ...
    async cleanupAllFiles() {
        const cleanupPromises = this.jobs.map((job) => rmAsync(job.path, { force: true }).catch((e) => console.error(`Failed to delete ${job.path}`, e)));
        ...
        if (this.currentJobPath) {
            for (let i = 0; i < PARALLEL_CHUNK_COUNT; i++) {
                ...
                cleanupPromises.push(rmAsync(chunkPath, { force: true }).catch((e) => console.error(`Failed to delete chunk file ${chunkPath}`, e)));
            }
        }
            ...
            for (const part of this.parts) {
                if (part.useChunks) {
                    for (let i = 0; i < PARALLEL_CHUNK_COUNT; i++) {
                        ...
                        cleanupPromises.push(rmAsync(chunkPath, { force: true }).catch((e) => console.error(`Failed to delete part chunk file ${chunkPath}`, e)));
                    }
                }
            }
        }
