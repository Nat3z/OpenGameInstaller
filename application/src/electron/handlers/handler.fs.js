...
export default function handler() {
    ...
    ipcMain.handle('fs:move', async (_, arg) => {
    ...
    ipcMain.handle('fs:extract-rar', async (_, arg) => {
        ...
        if (process.platform === 'win32') {
            ...
            await new Promise((resolve, reject) => exec(`${s7ZipPath} x "${rarFilePath}" -o"${outputDir}"`, (err, stdout, stderr) => {
                if (err) {
                    console.error(err);
                    console.log(stderr);
                    if (downloadId) {
    ...
    ipcMain.on('fs:stat', async (event, arg) => {
    ...
    ipcMain.handle('fs:extract-zip', async (_, arg) => {
        try {
            ...
            if (process.platform === 'win32') {
                ...
                let s7ZipPath = '"C:\\Program Files\\7-Zip\\7z.exe"';
                await new Promise((resolve, reject) => exec(`${s7ZipPath} x "${zipFilePath}" -o"${outputDir}"`, (err, stdout, stderr) => {
                    if (err) {
                        console.error(err);
                        console.log(stderr);
                        if (downloadId) {
...
//# sourceMappingURL=handler.fs.js.map