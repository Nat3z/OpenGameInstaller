 ...
 */
export function getVersionedGameName(name, version) {
 ...
 */
export function getNonSteamGameAppID(gameName) {
    return new Promise((resolve) => {
        if (cachedAppIds[gameName]) {
            resolve({ success: true, appId: cachedAppIds[gameName] });
            return;
        }
        execFile(STEAMTINKERLAUNCH_PATH, ['getid', gameName], { cwd: __dirname }, (error, stdout, _stderr) => {
            if (error) {
            ...
            if (!appIdLine) {
                console.error('[getNonSteamGameAppID] Could not find app ID for game:', gameName);
                resolve({
                    success: false,
            ...
            console.log(`[getNonSteamGameAppID] Found app ID ${appId} for "${gameName}"`);
 ...
 */
export async function getSteamAppIdWithFallback(name, version, context) {
    ...
    if (!success) {
        ...
        if (fallbackResult.success) {
            ...
            console.log(`${contextPrefix}Found Steam app ID using plain name "${name}" after versioned lookup failed.`);
        }
    }
    return {
        success,
...
export async function addGameToSteam(params) {
    ...
    return new Promise((resolve) => exec(`${STEAMTINKERLAUNCH_PATH} addnonsteamgame --appname="${escapeShellArg(versionedGameName)}" --exepath="${escapeShellArg(params.launchExecutable)}" --startdir="${escapeShellArg(params.cwd)}" --launchoptions="${escapeShellArg(params.launchOptions)}" --compatibilitytool="proton_experimental" --use-steamgriddb`, {
        cwd: __dirname,
    }, (error, stdout, stderr) => {
        if (error) {
            console.error(error);
