...
import { loadLibraryInfo, saveLibraryInfo, ensureLibraryDir, ensureInternalsDir, getAllLibraryFiles, removeLibraryFile, addToInternalsApps, removeFromInternalsApps, } from './helpers.app/library.js';
...
export function registerLibraryHandlers(mainWindow) {
    ...
    ipcMain.handle('app:add-manual-game', async (_, data) => {
        ensureLibraryDir();
        ensureInternalsDir();
        const gameData = {
            ...data,
    ...
    ipcMain.handle('app:remove-app', async (_, appid) => {
        ensureLibraryDir();
        ensureInternalsDir();
        const appInfo = loadLibraryInfo(appid);
        if (!appInfo) {
    ...
    ipcMain.handle('app:insert-app', async (_, data) => {
        ensureLibraryDir();
        ensureInternalsDir();
        ...
        if (isLinux()) {
            ...
            const hasRedistributables = data.redistributables && data.redistributables.length > 0;
            ...
            if (hasRedistributables) {
                console.log('[setup] Redistributables detected. Returning setup-prefix-required status.');
                console.log('[setup] User needs to restart Steam and launch the game to create Proton prefix.');
                ...
                for (const redistributable of data.redistributables) {
                    try {
                        if (!fs.existsSync(redistributable.path)) {
                            throw new Error(`Redistributable path does not exist: ${redistributable.path}`);
                        }
                        ...
                        console.error(`[redistributable] failed to install ${redistributable.name} for ${data.name}: ${error}`);
                    }
                }
    ...
    ipcMain.handle('app:update-app-version', async (_, data) => {
        ...
        if (isLinux()) {
            ...
            const { success, appId } = await getSteamAppIdWithFallback(appData.name, appData.version, 'app:update-app-version');
            if (success && appId) {
                ...
                console.warn(`[app:update-app-version] Failed to get Steam app ID for "${getVersionedGameName(appData.name, appData.version)}"${appData.version ? ` and fallback "${appData.name}"` : ''}. Preserving original launch arguments (including any existing WINEPREFIX).`);
            }
        }
        else {
...
//# sourceMappingURL=library-handlers.js.map