...
export function registerRedistributableHandlers() {
    ipcMain.handle('app:install-redistributables', async (_, appID) => {
        if (!isLinux()) {
            return 'failed';
        }
        ...
        const { success, appId } = await getSteamAppIdWithFallback(appInfo.name, appInfo.version, 'redistributable');
        if (!success || !appId) {
            return 'failed';
        }
        let protonPath;
        try {
            ...
            console.error('[redistributable] Error getting Proton prefix path:', error);
            sendNotification({
                message: 'Failed to get Proton prefix path. Please check your Steam installation.',
        ...
        if (!fs.existsSync(protonPath)) {
            console.error('[redistributable] Proton prefix does not exist:', protonPath);
            sendNotification({
                message: 'Proton prefix not found. Please launch the game through Steam first.',
                id: generateNotificationId(),
                type: 'error',
            });
            return 'failed';
        }
        console.log(`[redistributable] Installing ${appInfo.redistributables.length} redistributables for ${appInfo.name}`);
        ...
        for (const redistributable of appInfo.redistributables) {
            try {
                ...
                console.log('[redistributable] Installing:', redistributable.name, redistributable.path);
                const success = await new Promise((resolve) => {
                    ...
                    (async () => {
                        try {
                            if (redistributable.path === 'microsoft' &&
                                redistributable.name === 'dotnet-repair') {
                                // Download and run the .NET Framework Repair Tool
                                console.log('[dotnet-repair] Starting .NET Framework Repair Tool');
                                const netfxRepairToolUrl = 'https://download.microsoft.com/download/2/b/d/2bde5459-2225-48b8-830c-ae19caf038f1/NetFxRepairTool.exe';
                                ...
                                if (!fs.existsSync(toolPath)) {
                                    console.log('[dotnet-repair] Downloading .NET Framework Repair Tool...');
                                    ...
                                    await new Promise((downloadResolve, downloadReject) => {
                                        ...
                                        fileStream.on('error', (err) => {
                                            ...
                                            try {
                                                ...
                                                console.error('[dotnet-repair] Failed to cleanup file:', unlinkErr);
                                            }
                                            downloadReject(err);
                                        });
                                    });
                                ...
                                console.log('[dotnet-repair] Running .NET Framework Repair Tool with wine');
                                ...
                                child.on('close', (code) => {
                                    console.log(`[dotnet-repair] process exited with code ${code}`);
                                    finalize(code === 0);
                                });
                                ...
                                const silentFlags = getSilentInstallFlags(redistributablePath, redistributableFilename);
                                const child = spawn('flatpak', [
                                    `--env=WINEPREFIX=${protonPath}`,
                                    `--env=DISPLAY=:0`,
                                    `--env=WINEDEBUG=-all`,
...
//# sourceMappingURL=redistributable-handlers.js.map