...
import { isLinux, getHomeDir, getProtonPrefixPath, } from './helpers.app/platform.js';
import { getSteamAppIdWithFallback, getNonSteamGameAppID, getVersionedGameName, addGameToSteam, } from './helpers.app/steam.js';
import { loadLibraryInfo, saveLibraryInfo, ensureLibraryDir, } from './helpers.app/library.js';
import { generateNotificationId } from './helpers.app/notifications.js';
import { sendNotification } from '../main.js';
export function registerSteamHandlers() {
    ipcMain.handle('app:get-steam-app-id', async (_, appID) => {
        if (!isLinux()) {
        ...
        return await getSteamAppIdWithFallback(appInfo.name, appInfo.version, 'app:get-steam-app-id');
    });
    ...
    ipcMain.handle('app:launch-steam-app', async (_, appID) => {
        if (!isLinux()) {
            return { success: false, error: 'Only available on Linux' };
        }
        const appInfo = loadLibraryInfo(appID);
        ...
        const { success, appId } = await getSteamAppIdWithFallback(appInfo.name, appInfo.version, 'steam');
        if (!success) {
    ...
    ipcMain.handle('app:check-prefix-exists', async (_, appID) => {
        if (!isLinux()) {
        ...
        const { success, appId } = await getSteamAppIdWithFallback(libraryInfo.name, libraryInfo.version, 'prefix');
    ...
    ipcMain.handle('app:add-to-steam', async (_, appID, oldSteamAppId) => {
        if (!isLinux()) {
            return { success: false, error: 'Only available on Linux' };
        }
        ensureLibraryDir();
        ...
        const { success, appId: newSteamAppId } = await getNonSteamGameAppID(versionedGameName);
                ...
                try {
                    ...
                    if (fs.existsSync(oldAppIdDir)) {
                        ...
                        if (fs.existsSync(newAppIdDir)) {
                            console.warn(`[add-to-steam] New compatdata directory already exists at ${newAppIdDir}, skipping migration`);
                            ...
                            try {
                                ...
                                console.log(`[add-to-steam] Successfully migrated compatdata directory from ${oldAppIdDir} to ${newAppIdDir}`);
                                sendNotification({
                                ...
                                console.log(`[add-to-steam] Rename failed (possibly cross-filesystem), using copy instead`);
                                // Copy recursively
                                ...
                                console.log(`[add-to-steam] Successfully copied compatdata directory from ${oldAppIdDir} to ${newAppIdDir}`);
                                sendNotification({
                                    message: `Successfully migrated prefix to new version.`,
                                    id: generateNotificationId(),
                                    type: 'success',
                        ...
                        console.log(`[add-to-steam] Old compatdata directory not found at ${oldAppIdDir}, skipping migration`);
                        sendNotification({
                    ...
                    console.error(`[add-to-steam] Error migrating prefix:`, migrationError);
...
//# sourceMappingURL=steam-handlers.js.map