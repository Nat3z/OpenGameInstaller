        ...
        case 'qbittorrent': {
          try {
            ...
            qbitClient = new QBittorrent({
              ...
              username:
                (await getStoredValue('qbittorrent', 'qbitUsername')) ??
                'admin',
              password:
                (await getStoredValue('qbittorrent', 'qbitPassword')) ?? '',
            });
            ...
            const torrentInterval = setInterval(async () => {
              ...
              if (!mainWindow || !mainWindow.webContents) {
                console.error(
                  'Seems like the window is closed. Cannot send progress message to renderer.'
                );
                return;
              }
              mainWindow.webContents.send('torrent:download-progress', {
                id: downloadID,
                downloadSpeed,
                progress,
                fileSize,
              ...
              if (torrent.isCompleted && !alreadyNotified) {
                mainWindow.webContents.send('torrent:download-complete', {
                  id: downloadID,
                });
