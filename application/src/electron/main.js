...
import { checkForAddonUpdates, convertLibrary, IS_NIXOS, STEAMTINKERLAUNCH_PATH, } from './startup.js';
...
if (IS_NIXOS) {
    console.log('NixOS detected, but startup logic has been moved. If you have issues, please check startup.ts');
}
if (STEAMTINKERLAUNCH_PATH === '') {
    console.error('STEAMTINKERLAUNCH_PATH is empty. This should be handled in startup.ts');
}
console.log('STEAMTINKERLAUNCH_PATH: ' + STEAMTINKERLAUNCH_PATH);
console.log('Running in directory: ' + __dirname);
export let torrentIntervals = [];
let mainWindow;
...
let readyForEventWaiters = [];
export async function sendIPCMessage(channel, ...args) {
    if (!isReadyForEvents) {
...
export function sendAskForInput(id, config, name, description) {
    if (!mainWindow) {
...
function createWindow() {
    ...
    mainWindow.loadURL('file://' + join(app.getAppPath(), 'public', 'splash.html'));
...
async function startAppFlow(win) {
