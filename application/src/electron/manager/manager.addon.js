...
export async function setupAddon(addonPath) {
    ...
    const addonJSON = JSON.parse(addonConfig);
...
> ${addon.scripts.preSetup}
      `;
            setupLogs += await executeScript('pre-setup', addon.scripts.preSetup, addonPath, addonName);
        }
        catch (e) {
            sendNotification({
                type: 'error',
...
> ${addon.scripts.postSetup}
      `;
            await executeScript('post-setup', addon.scripts.postSetup, addonPath, addonName);
        }
        catch (e) {
            sendNotification({
                type: 'error',
...
export async function startAddon(addonPath, addonLink) {
    const addonConfig = await readFile(join(addonPath, 'addon.json'), 'utf-8');
    ...
    const addon = AddonFileConfigurationSchema.parse(addonJSON);
    try {
        executeScript('run', addon.scripts.run + ' --addonSecret=' + addonSecret, addonPath, addonName);
        let attempts = 0;
        ...
        if (client) {
            ...
            console.log('Registered addon identifier path for ' +
                addonName +
                ' to ' +
                addonPath +
                ' with link ' +
        ...
        await writeFile(join(addonPath, 'run-crash.log'), stripAnsiCodes(errorMessage));
        sendNotification({
            type: 'error',
...
async function executeScript(scriptName, script, addonPath, addonName) {
    return new Promise((resolve, reject) => {
        ...
        finalScript = finalScript.replace(/^(\.?[\\/]?bun(?:.exe)?)\b/, `"${bunPath}"`);
        ...
        child.stderr?.on('data', (data) => {
            console.error('[' + addonName + '@' + scriptName + '] ' + data.toString());
            stderr += data.toString();
        });
        child.on('close', (code) => {
            if (code !== 0) {
                // write the error to a log file
                console.error('[' + addonName + '@' + scriptName + '] Exited with error: ' + code);
...
//# sourceMappingURL=manager.addon.js.map