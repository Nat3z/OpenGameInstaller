...
let migrations = {
    'install-steam-addon': {
        ...
        platform: 'all',
        run: async () => {
            const generalConfig = await fs.readFile(join(__dirname, 'config/option/general.json'), 'utf-8');
            const generalConfigObj = JSON.parse(generalConfig);
            const addons = generalConfigObj.addons;
            const hasSteamAddon = addons.find((addon) => addon.includes('Nat3z/steam-integration')) !== undefined;
            if (!hasSteamAddon) {
                addons.push('https://github.com/Nat3z/steam-integration');
            }
            await fs.writeFile(join(__dirname, 'config/option/general.json'), JSON.stringify(generalConfigObj));
    ...
    'install-steam-addon-repair': {
        from: '2.0.0',
        to: '2.1.0',
        description: 'checks if the steam-addon was installed without an installation.log file and if so, repairs it.',
        platform: 'all',
        run: async () => {
            const generalConfig = await fs.readFile(join(__dirname, 'config/option/general.json'), 'utf-8');
            const generalConfigObj = JSON.parse(generalConfig);
            ...
            const hasSteamAddon = addons.find((addon) => addon.includes('Nat3z/steam-integration')) !== undefined;
            if (!hasSteamAddon) {
                console.log('user does not have steam-integration in config. no need to repair.');
                return;
            }
            // check if installation.log exists in the addon path
            const addonPath = join(__dirname, 'addons', 'steam-integration', 'installation.log');
    ...
    'install-flatpak-wine': {
        from: '2.1.2',
        to: '2.2.0',
        description: 'Installs flatpak wine if not already installed on Linux systems.',
        platform: 'linux',
        run: async () => {
            // Check if flatpak is installed
            const flatpakInstalled = await new Promise((resolve) => {
            ...
            if (!flatpakInstalled) {
                console.log('[migration] flatpak not available, skipping wine installation');
                return;
            }
            // Check if wine is already installed
            const wineInstalled = await new Promise((resolve) => {
            ...
            const result = await new Promise((resolve) => {
                ...
                const childProcess = spawn('flatpak', [
                    'install',
                    '--system',
                    '-y',
                    'flathub',
                ...
                childProcess.on('close', (code) => {
                    console.log('[migration] wine install process exited with code:', code);
                    if (code !== 0) {
            ...
            const wineVerification = await new Promise((resolve) => {
                exec('flatpak run org.winehq.Wine --help', (err) => {
                    if (err) {
    ...
    'add-to-desktop-2.5.0': {
        ...
        run: async () => {
            await addToDesktop();
            sendNotification({
                message: 'Desktop shortcut created successfully. You can now find OpenGameInstaller in your Desktop.',
                id: Math.random().toString(36).substring(7),
                type: 'success',
            });
        },
    ...
    'migrate-update-state-format': {
        from: '0.0.0',
        to: '2.6.0',
        description: 'Migrates update-state.json from old format (array of numbers) to new format (array of objects with appID and steamAppId)',
        platform: 'all',
        run: async () => {
            const updateStatePath = join(__dirname, 'internals/update-state.json');
            // Skip if file doesn't exist
            if (!fsSync.existsSync(updateStatePath)) {
                console.log('[migration] update-state.json does not exist, skipping migration');
                return;
            }
            try {
                ...
                if (Array.isArray(parsed.requiredReadds) &&
                    ...
                    console.log('[migration] update-state.json already in new format, skipping migration');
                    return;
                }
                // Check if it has any old format entries (array containing numbers)
                if (Array.isArray(parsed.requiredReadds)) {
                    const hasOldFormat = parsed.requiredReadds.some((v) => typeof v === 'number' && Number.isFinite(v));
                    if (hasOldFormat) {
                        console.log('[migration] Converting update-state.json from old format to new format');
                        ...
                        const oldCount = parsed.requiredReadds.length - convertedRequiredReadds.length;
                        if (oldCount > 0) {
                            console.log(`[migration] Removed ${oldCount} old format entries (steamAppId unknown)`);
                        }
                        const migratedData = {
                            requiredReadds: convertedRequiredReadds,
                        };
                        await fs.writeFile(updateStatePath, JSON.stringify(migratedData, null, 2));
                    ...
                    console.log('[migration] update-state.json format is unrecognized, skipping migration');
                }
            }
            catch (error) {
...
export async function execute() {
    ...
    if (fsSync.existsSync(join(__dirname, 'config/option/lastVersion.txt'))) {
        lastVersion = await fs.readFile(join(__dirname, 'config/option/lastVersion.txt'), 'utf-8');
    }
