...
app.post('/:addonID/config', async (req, res) => {
  ...
  if (response.args.success) res.json({ success: true });
  else {
...
app.get('/:addonID/search', async (req, res) => {
  ...
  const deferrableTask = new DeferrableTask(async () => {
    const event = await client.sendEventMessage({
      event: 'search',
      args: {
        text: req.query.gameID ?? req.query.steamappid,
        type: req.query.gameID
          ? 'internal'
          : req.query.steamappid
            ? 'steamapp'
            : '',
      },
    });
...
app.post('/:addonID/request-dl', async (req, res) => {
  ...
  const deferrableTask = new DeferrableTask(async () => {
    const data = await client.sendEventMessage({
      event: 'request-dl',
      args: { appID: req.body.appID, info: req.body.info },
    });
    return data.args;
  }, client.addonInfo.id);
...
app.post('/:addonID/setup-app', async (req, res) => {
  ...
  if (
    !req.body ||
    req.body.usedRealDebrid === undefined ||
    typeof req.body.usedRealDebrid !== 'boolean'
  ) {
    return res.status(400).send('No usedRealDebrid provided');
  }
  if (
    !req.body ||
    req.body.storefront === undefined ||
    typeof req.body.storefront !== 'string'
  ) {
  ...
  const deferrableTask = new DeferrableTask(async () => {
    const data = await client.sendEventMessage({
      ...
      args: {
        ...
        type: req.body.type,
        usedRealDebrid: req.body.usedRealDebrid,
        storefront: req.body.storefront,
        name: req.body.name,
        multiFiles: req.body.multiPartFiles,
