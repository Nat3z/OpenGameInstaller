...
import { restoreBackup, removeCachedAppUpdates, reinstallAddonDependencies, } from './startup.js';
 ...
 */
function updateSplashStatus(text, subtext) {
 ...
 */
function updateSplashProgress(current, total, speed) {
    if (splashTargetWindow && !splashTargetWindow.isDestroyed()) {
 ...
 */
export async function runStartupTasks(mainWindow) {
    try {
        ...
        if (backupResult.needsAddonReinstall) {
            ...
            try {
                await reinstallAddonDependencies((addonName, current, total) => {
                    updateSplashStatus('Installing addon dependencies', `${addonName} (${current}/${total})`);
                    updateSplashProgress(current, total, '');
                });
            }
            catch (error) {
...
//# sourceMappingURL=startup-runner.js.map