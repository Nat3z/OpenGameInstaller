...
import { chmodSync, createWriteStream, existsSync, mkdirSync, readFileSync, readdirSync, rmSync, statSync, copyFileSync, writeFileSync, } from 'original-fs';
 ...
 */
function countFilesToBackup(sourcePath) {
...
async function* copyDirectoryAsync(source, destination) {
    ...
    try {
        ...
        console.error(`[updater] Failed to read directory ${source}: ${error.message}`);
        yield { file: source, success: false, error: error.message };
    }
}
/**
 ...
 */
async function backupFilesAsync(tempFolder, updateStatus, updateProgress) {
    ...
    for (const file of filesToBackup) {
        ...
        for await (const result of copyDirectoryAsync(source, destination)) {
            ...
            if (copiedFiles % 10 === 0 || copiedFiles === totalFiles) {
                const progress = totalFiles > 0 ? Math.round((copiedFiles / totalFiles) * 100) : 100;
                updateStatus('Backing up files', `${progress}% (${copiedFiles}/${totalFiles})`);
    ...
    if (failedFiles.length > 0) {
        console.warn(`[updater] Failed to backup ${failedFiles.length} files:`, failedFiles.slice(0, 10));
    }
    return {
                ...
                if (error) {
                    ...
                    exec('killall -q OpenGameInstaller-Setup.AppImage', (error2) => {
                        if (error2) {
                            ...
                            console.log('[updater] Killed Setup.AppImage process via killall');
                        }
                        resolve();
                    });
                }
 ...
 */
export function checkIfInstallerUpdateAvailable(callbacks) {
    return new Promise(async (resolve) => {
        ...
        const updateProgress = (current, total, speed) => {
            if (callbacks) {
        ...
        try {
            ...
            const releases = await axios.get(`https://api.github.com/repos/${gitRepo}/releases`, { timeout: 10000 } // 10 second timeout for update check
            );
            ...
            let latestSetupVersionUrl = undefined;
            const latestVersion = latestRelease.tag_name;
            if (process.platform === 'win32') {
                latestSetupVersionUrl = latestRelease.assets.find((asset) => asset.name.includes('-Setup.exe'))?.browser_download_url;
            }
            else if (process.platform === 'linux') {
                latestSetupVersionUrl = latestRelease.assets.find((asset) => asset.name.includes('-Setup.AppImage'))?.browser_download_url;
            }
            ...
            if (latestSetupVersion !== localVersion) {
                ...
                if (process.platform === 'win32') {
                    ...
                    response.data.on('data', () => {
                        ...
                        updateStatus('Downloading Latest Setup', correctParsingSize(downloadSpeed) + '/s');
                        updateProgress(writer.bytesWritten, fileSize, correctParsingSize(downloadSpeed) + '/s');
                    });
                    writer.on('finish', async () => {
                        ...
                        try {
                            ...
                            const backupResult = await backupFilesAsync(tempFolder, updateStatus, updateProgress);
                            if (backupResult.needsAddonReinstall) {
                                // Create a flag file to trigger addon reinstall on next launch
                                try {
                                    const flagPath = join(tempFolder, 'needs-addon-reinstall.flag');
                                    writeFileSync(flagPath, new Date().toISOString());
                                    console.log('[updater] Created addon reinstall flag');
                                }
                                catch (flagError) {
                                    console.warn('[updater] Failed to create reinstall flag:', flagError.message);
                                }
                            }
                            if (!backupResult.success) {
                                console.warn('[updater] Backup completed with some failures, but continuing...');
                            }
                        }
                        catch (backupError) {
                    ...
                    writer.on('finish', async () => {
                        ...
                        setTimeout(async () => {
                            try {
                                ...
                                copyFileSync('../temp-setup-OGI.AppImage', '../OpenGameInstaller-Setup.AppImage');
                            ...
                            updateStatus('Shutting Down OpenGameInstaller', 'Please open OpenGameInstaller again');
                            await setTimeoutPromise(3000);
                            process.exit(0);
                        }, 500);
                    });
