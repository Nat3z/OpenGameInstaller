...
<div
  class="w-full h-full fixed bg-slate-900/50 flex justify-center items-center top-0 left-0 z-10"
>
  <!-- close button at top right-->
  <article
    class="w-3/6 h-5/6 bg-slate-100 p-4 py-2 border-gray-200 animate-fade-in-pop rounded-lg relative"
  >
    <button
      class="absolute top-4 right-4 text-white hover:bg-slate-200 px-2 py-2 border-none rounded-lg"
    ...
    <div class="options">
      ...
      {#each Object.keys(screenRendering) as key}
        <div class="flex flex-col items-start justify-center relative">
          <div class="flex flex-row gap-2 items-center relative">
            ...
            {#if isStringOption(screenRendering[key])}
              {#if screenRendering[key].allowedValues.length !== 0}
                <select
                  data-input
                  id={key}
                  value={screenRendering[key].defaultValue ||
                    screenRendering[key].allowedValues[0]}
                >
              ...
              {:else if screenRendering[key].inputType === "text" || screenRendering[key].inputType === "password"}
                <input
                  data-input
                  type={screenRendering[key].inputType}
                  value={screenRendering[key].defaultValue ?? ""}
                  id={key}
                  maxlength={screenRendering[key].maxTextLength}
                  minlength={screenRendering[key].minTextLength}
                />
              {:else if screenRendering[key].inputType === "file" || screenRendering[key].inputType === "folder"}
                ...
                {:else}
                  <button
                    class="bg-blue-500 text-white px-2 rounded"
                    onclick={(ev) => browseForFolder(ev, "file")}>Browse</button
                  >
                {/if}
              {/if}
            {/if}
            {#if isNumberOption(screenRendering[key])}
              ...
              {#if screenRendering[key].inputType === "range"}
                <p>
                  {screenRendering[key].defaultValue ??
                    screenRendering[key].min}
                </p>
              {/if}
            {/if}
          ...
          <h2 class="block text-xs text-gray-300">
            {screenRendering[key].description}
          </h2>
        </div>
      {/each}
      <div class="flex flex-row justify-start items-center gap-2">
        <button
          onclick={pushChanges}
          class="px-4 py-1 rounded w-fit border-none bg-blue-300 mt-4"
          >Save</button
        >
        <button
          onclick={removeFromList}
          class="px-4 py-1 rounded w-fit border-none bg-red-300 mt-4"
          >Remove</button
        >
      </div>
    </div>
