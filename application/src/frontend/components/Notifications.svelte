<script lang="ts">
  ...
  let timers = new Map<string, { timer: ReturnType<typeof setTimeout>; progressTimer: ReturnType<typeof setInterval> }>();
  ...
  function startNotificationTimer(notificationId: string) {
    ...
    const timer = setTimeout(() => {
      ...
      element.animate([{ opacity: 1 }, { opacity: 0 }], { duration: fadeDuration, fill: "forwards" });
      setTimeout(() => {
...
<div class="fixed bottom-2 right-2 gap-3 w-5/6 flex justify-end items-end flex-col-reverse pointer-events-none z-50">
    ...
    >
      <div class="flex items-center gap-3 pb-3">
        <div class="flex-shrink-0">
          <img src={`./${notification.type}.svg`} alt={notification.type} class="w-5 h-5 notification-icon" />
        </div>
        <p class="notification-text font-open-sans text-sm font-medium leading-relaxed flex-1">{notification.message}</p>
      </div>
      <div class="progress-bar">
        <div class="progress-fill" style="width: {100 - (progressValues.get(notification.id) || 0)}%"></div>
      </div>
    </div>
  {/each}
</div>