 ...
 */
export async function startDownload(result, appID, event, htmlButton) {
    ...
    if (downloadHandler === 'torrent' || downloadHandler === 'magnet') {
        const generalOptions = getConfigClientOption('general');
        const torrentClient = (generalOptions ? generalOptions.torrentClient : null) ?? 'disable';
 ...
 */
export function updateDownloadStatus(downloadID, updates) {
 ...
 */
export function getDownloadItem(downloadID) {
