...
import { deleteDownloadedItems, deletePersistedDownload, } from '../downloads/persistence';
const pausedDownloadStates = new Map();
// Track which resume operations have already triggered bulk queueing to avoid duplicates
const queuedAfterResume = new Set();
async function enqueueRemainingPausedDownloads(resumedId, pausedStates) {
    if (queuedAfterResume.has(resumedId))
        return;
    queuedAfterResume.add(resumedId);
    // Build an ordered list of other paused items to enqueue behind the active one
    let downloadsSnapshot = [];
    currentDownloads.subscribe((d) => (downloadsSnapshot = d))();
    const toQueue = downloadsSnapshot.filter((d) => d.id !== resumedId && d.status === 'paused');
    // Enqueue sequentially to preserve order
    for (const item of toQueue) {
        try {
            // Skip if we lack the data needed to restart
            const itemDownloadURL = item.downloadType === 'torrent' || item.downloadType === 'magnet'
                ? item.downloadURL
                : undefined;
            const effectiveUrl = item.usedDebridService
                ? itemDownloadURL || item.originalDownloadURL
                : item.originalDownloadURL || itemDownloadURL;
            const hasFiles = item.downloadType === 'direct' &&
            ...
            console.error('Failed to enqueue paused download after resume:', item.id, e);
        }
    }
}
export async function pauseDownload(downloadId) {
    try {
        ...
        else if (download.downloadType === 'torrent' ||
            download.downloadType === 'magnet') {
            try {
    ...
    try {
        ...
        if (!pausedState) {
            ...
            pausedState = {
                ...
                pausedAt: Date.now(),
                originalDownloadURL: fallback.originalDownloadURL ||
        ...
        console.log('Resuming download:', download.name, 'Type:', download.downloadType);
        updateDownloadStatus(downloadId, { status: 'downloading' });
        ...
        if (wasReconstructed) {
            console.log('Reconstructed paused state with no backend context; restarting download instead of resuming.');
...
export function cancelPausedDownload(downloadId) {
    try {
