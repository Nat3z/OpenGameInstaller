...
async function restartDirectDownload(download) {
    // Prefer the latest resolved URL for Debrid services. Fall back otherwise.
    const downloadURL = download.downloadType === 'torrent' || download.downloadType === 'magnet'
    ...
    let files = [];
    const downloadFiles = download.downloadType === 'direct' ? download.files : undefined;
    if (downloadFiles && downloadFiles.length > 0) {
        ...
        const isFilePath = typeof download.downloadPath === 'string' &&
        ...
        if (isFilePath) {
            ...
            const downloadFilename = download.downloadType === 'torrent' ||
                download.downloadType === 'magnet'
                ? download.filename
                : undefined;
            const filenameHasExt = !!downloadFilename && /\.[A-Za-z0-9]{1,8}$/.test(downloadFilename);
            const chosenFilename = (urlFilename && /\.[A-Za-z0-9]{1,8}$/.test(urlFilename)
                ? urlFilename
                : undefined) ||
...
async function restartTorrentDownload(download) {
    ...
    let filename = download.downloadType === 'torrent' || download.downloadType === 'magnet'
        ? download.filename
        : undefined;
    if (!filename) {
    ...
    if (download.downloadType === 'torrent') {
        ...
        throw new Error(`Unsupported torrent download type: ${download.downloadType}`);
    }
}
export async function restartDownload(pausedState, pausedDownloadStates) {
    try {
