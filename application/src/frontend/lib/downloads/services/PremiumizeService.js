import { getConfigClientOption, getDownloadPath, listenUntilDownloadReady, } from '../../../utils';
import { BaseService } from './BaseService';
const BASE_URL = 'https://www.premiumize.me/api';
export class PremiumizeService extends BaseService {
    types = ['premiumize-magnet', 'premiumize-torrent'];
    async startDownload(result, appID, _event, htmlButton) {
        ...
        try {
            ...
            const optionHandled = getConfigClientOption('realdebrid');
            ...
            const responseFolderData = responseFolder.data;
            if (responseFolderData.status === 'success') {
                ...
                const responseFolderContentData = responseFolderContent.data;
                if (responseFolderContentData.status === 'success') {
                    const folder = responseFolderContentData.content.find((item) => item.name === 'OpenGameInstaller');
                    if (folder) {
                        folderId = folder.id;
                    }
                    else {
            ...
            if (result.downloadType === 'torrent') {
                const torrentData = await window.electronAPI.downloadTorrentInto(result.downloadURL);
                formDataAddTorrent.append('file', new Blob([torrentData.buffer]));
            }
            ...
            const responseAddTorrent = await window.electronAPI.app.axios({
                method: 'POST',
            ...
            const foundFolderId = await new Promise((resolve, reject) => {
                let attempts = 0;
                const interval = setInterval(async () => {
                    try {
                        ...
                        const responseTransfersList = await window.electronAPI.app.axios({
                            method: 'GET',
                        ...
                        const transfer = responseTransfersList.data.transfers.find((transfer) => transfer.id === transferId);
            ...
            const responseTorrentFile = await window.electronAPI.app.axios({
                method: 'POST',
            ...
            this.updateDownloadRequested(downloadID, tempId, directDownloadUrl, getDownloadPath() + '/' + result.name + '/' + result.filename + '.zip', 'premiumize', updatedState, result);
        }
        finally {
            if (htmlButton) {
...
//# sourceMappingURL=PremiumizeService.js.map