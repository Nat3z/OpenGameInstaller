...
export class TorboxService extends BaseService {
    types = ['torbox-magnet', 'torbox-torrent'];
    async startDownload(result, appID, event, htmlButton) {
        if (event === null && !htmlButton)
            return;
        ...
        try {
            const optionHandled = getConfigClientOption('torbox');
            ...
            if (result.downloadType === 'torrent') {
                ...
                addTorrentForm.append('file', new Blob([torrentData.buffer]));
                ...
                torrentHash = await window.electronAPI.getTorrentHash(result.downloadURL);
            }
            ...
            if (response.status !== 200) {
                ...
                const message = response.data.error === 'DOWNLOAD_TOO_LARGE'
                    ...
                    : response.data.detail.includes('active torrent limit of')
                        ...
                        : response.data.detail.includes('reached your monthly download limit')
            ...
            if ('cooldown_until' in responseData) {
                ...
                throw new Error('You are on a cooldown period. Please wait until ' +
            ...
            if (!finalTorrentId) {
                const torrentGrabber = await new Promise((resolve) => {
                    ...
                    interval = setInterval(async () => {
                        try {
                            ...
                            if (Date.now() - startTime > timeoutMs) {
                                cleanup();
                                console.error('Timeout: Torrent did not appear on Torbox within the allowed time');
                                resolve(undefined);
                                return;
                            }
                            const torrentInfo = await window.electronAPI.app.axios({
                            ...
                            const torrent = torrentInfo.data.data.find((torrent) => torrent.hash === torrentHash);
                            if (!torrent) {
                                return;
                            }
            ...
            if (finalTorrentId) {
                ...
                if (!filename) {
                    currentDownloads.update((downloads) => downloads.filter((download) => download.id !== tempId));
                ...
                this.updateDownloadRequested(downloadID, tempId, downloadUrl, downloadPath, 'torbox', updatedState, result);
            }
            else {
...
//# sourceMappingURL=TorboxService.js.map