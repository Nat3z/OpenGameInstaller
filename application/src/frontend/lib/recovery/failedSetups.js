import { createNotification, currentDownloads, failedSetups, setupLogs, } from '../../store';
import { unrarAndReturnOutputDir, unzipAndReturnOutputDir, } from '../setup/extraction';
...
export function removeFailedSetup(setupId) {
    try {
...
export function saveFailedSetup(setupInfo) {
    try {
        ...
        window.electronAPI.fs.write(`./failed-setups/${failedSetupId}.json`, JSON.stringify(failedSetupData, null, 2));
        failedSetups.update((setups) => {
            const index = setups.findIndex((s) => (s.downloadInfo && s.downloadInfo.id) === failedSetupId);
            if (index !== -1) {
...
export async function retryFailedSetup(failedSetup) {
    const updateRetry = (newSetup, error) => {
        const updatedSetup = {
            ...newSetup,
            id: failedSetup.id,
        ...
        failedSetups.update((setups) => setups.map((setup) => setup.id === failedSetup.id ? updatedSetup : setup));
    };
    try {
        ...
        failedSetups.update((setups) => setups.filter((setup) => setup.id !== failedSetup.id));
        ...
        if (failedSetup.should === 'call-unrar') {
            const filename = failedSetup.downloadInfo.downloadType === 'torrent' ||
                failedSetup.downloadInfo.downloadType === 'magnet'
                ? failedSetup.downloadInfo.filename
                : undefined;
            if (!filename) {
                throw new Error('Cannot extract RAR: filename not available for this download type');
            }
            const rarFilePath = failedSetup.downloadInfo.downloadPath.replace(/(\/|\\)$/g, '') +
                '/' +
                filename;
            const outputBase = failedSetup.downloadInfo.downloadPath.replace(/(\/|\\)$/g, '') +
                '/' +
        ...
        if (failedSetup.should === 'call-unzip') {
            ...
            const filename = failedSetup.downloadInfo.downloadType === 'torrent' ||
                ...
                : undefined;
            if (!filename) {
                throw new Error('Cannot extract ZIP: filename not available for this download type');
            }
            const originalZipFilePath = failedSetup.downloadInfo.downloadPath.replace(/(\/|\\)$/g, '') +
                '/' +
                filename;
        ...
        try {
            await runSetupApp({
                ...failedSetup.downloadInfo,
                id: tempId,
            }, setupData.path, failedSetup.downloadInfo.downloadType === 'torrent' ||
                failedSetup.downloadInfo.downloadType === 'magnet');
...
//# sourceMappingURL=failedSetups.js.map