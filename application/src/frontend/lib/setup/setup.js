import { createNotification, setupLogs, protonPrefixSetups, } from '../../store';
...
function dispatchSetupEvent(eventType, downloadID, data) {
    document.dispatchEvent(new CustomEvent(`setup:${eventType}`, {
        detail: {
            id: downloadID,
...
export function createSetupPayload(downloadedItem, path, forType, currentLibraryInfo, additionalData = {}) {
    return {
        ...
        multiPartFiles: JSON.parse(JSON.stringify(downloadedItem.downloadType === 'direct'
...
export function handleSetupError(error, downloadedItem, forType = 'game', currentLibraryInfo) {
    ...
    const baseData = {
        ...
        appID: downloadedItem.appID,
        storefront: downloadedItem.storefront,
        multiPartFiles: downloadedItem.downloadType === 'direct'
            ? downloadedItem.files
            : undefined,
        manifest: downloadedItem.manifest || {},
    };
    const setupData = forType === 'update' && currentLibraryInfo
        ? {
            ...baseData,
            for: 'update',
            currentLibraryInfo,
...
export function createSetupCallbacks(downloadedItem, forType = 'game', currentLibraryInfo) {
    return {
        onLogs: (log) => dispatchSetupEvent('log', downloadedItem.id, log),
        onProgress: (progress) => dispatchSetupEvent('progress', downloadedItem.id, progress),
        onFailed: (error) => handleSetupError(error, downloadedItem, forType, currentLibraryInfo),
        consume: 'json',
    };
}
export async function runSetupApp(downloadedItem, outputDir, isTorrent, additionalData = {}) {
    const setupPayload = createSetupPayload(downloadedItem, outputDir, 'game', undefined, additionalData);
    const callbacks = createSetupCallbacks(downloadedItem, 'game');
    try {
        const data = await safeFetch('setupApp', setupPayload, callbacks);
        ...
        if (result === 'setup-prefix-required') {
            console.log('[setup] Proton prefix setup required for:', downloadedItem.name);
 ...
 */
export async function runSetupAppUpdate(downloadedItem, outputDir, isTorrent, additionalData = {}) {
    const currentLibraryInfo = getApp(downloadedItem.appID);
    if (!currentLibraryInfo) {
        console.error(`[runSetupAppUpdate] Library entry not found for appID: ${downloadedItem.appID}`);
        updateDownloadStatus(downloadedItem.id, {
            status: 'error',
            error: `App not found in library (appID: ${downloadedItem.appID})`,
        });
        throw new Error(`App not found in library (appID: ${downloadedItem.appID})`);
    }
    const setupPayload = createSetupPayload(downloadedItem, outputDir, 'update', currentLibraryInfo, additionalData);
    const callbacks = createSetupCallbacks(downloadedItem, 'update', currentLibraryInfo);
    try {
        ...
        const result = await window.electronAPI.app.updateAppVersion(downloadedItem.appID, data.version, data.cwd, data.launchExecutable, data.launchArguments, downloadedItem.addonSource);
        ...
        if (data.version === downloadedItem.updateVersion) {
            ...
            if ((await window.electronAPI.app.getOS()) === 'linux' &&
                ...
                const steamAppIdResult = await window.electronAPI.app.getSteamAppId(downloadedItem.appID);
                ...
                if (steamAppId !== undefined) {
                    appUpdates.requiredReadds = [
                        ...appUpdates.requiredReadds.filter((r) => r.appID !== downloadedItem.appID),
                        { appID: downloadedItem.appID, steamAppId },
                    ];
                }
                else {
                    console.warn(`[setup] Failed to get Steam app ID for app ${downloadedItem.appID}, skipping prefix migration tracking`);
                    appUpdates.requiredReadds = appUpdates.requiredReadds.filter((r) => r.appID !== downloadedItem.appID);
                }
            }
        }
        else {
            ...
            console.log('Failed to update app. Target version: ', data.version, 'Current version: ', downloadedItem.updateVersion);
            updateDownloadStatus(downloadedItem.id, {
                status: 'error',
