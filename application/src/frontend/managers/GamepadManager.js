export class GamepadNavigator {
    focusableElements = [];
    currentElement = null; // Explicit tracking
    deadzone = 0.3; // Ignore small analog stick movements
    releaseThreshold = 0.15; // Lower threshold for detecting stick release (hysteresis)
    repeatDelay = 250; // ms between repeated navigation when held
    initialDelay = 400; // ms before first repeat when held
    lastNavigationTime = 0;
    isNavigating = false; // Track if we're currently in a navigation
    currentDirection = null; // Track active navigation direction
    gamepadLoop = null;
    ...
    refreshFocusableElements() {
        this.focusableElements = Array.from(document.querySelectorAll('button, [role="button"], a, input, select, textarea, [tabindex]:not([tabindex="-1"])')).filter((el) => {
    ...
    update() {
        ...
        if (Math.abs(rightStickX) > this.deadzone ||
            ...
            const scrollContainer = this.findScrollableAncestor(this.getCurrentElement());
            if (scrollContainer) {
    ...
    isElementInDirection(currentCenter, targetCenter, direction) {
    ...
    hasPerpendicularOverlap(currentRect, targetRect, direction) {
    ...
    getPerpendicularOffset(currentRect, targetRect, direction) {
        if (direction === 'up' || direction === 'down') {
    ...
    getEdgeDistance(currentRect, targetRect, direction) {
    ...
    calculateDistanceScore(currentRect, targetRect, direction) {
        // Get edge-to-edge distance (primary axis)
        let primaryDistance = this.getEdgeDistance(currentRect, targetRect, direction);
        ...
        const perpOffset = this.getPerpendicularOffset(currentRect, targetRect, direction);
    ...
    activateCurrentElement() {
        ...
        if (element.tagName === 'BUTTON' || element.tagName === 'A') {
            ...
            this.openSteamKeyboard(element);
        }
    }
    /**
     ...
     */
    async openSteamKeyboard(element) {
    ...
    destroy() {
        ...
        document.removeEventListener('focusin', this.handleExternalFocus.bind(this));
    }
}
