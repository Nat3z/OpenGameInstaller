...
<div class="flex flex-col overflow-y-auto justify-start overflow-x-hidden items-start w-full h-full relative">
  {#if focusedAddonId}
    <div class="absolute inset-0 w-full h-full z-10" in:fly={{ x: 100, duration: 400, easing: quintOut }} out:fly={{ x: -100, duration: 300 }}>
  ...
  {#if !focusedAddonId}
    <div class="relative w-full h-12 bg-background-color z-10" out:fly={{ y: -100, duration: 400, easing: quintOut }} in:fly={{ y: -100, duration: 400, easing: quintOut }}>
        ...
        {#if $view === "community-addons"}
          <div class="flex justify-center items-center">
            <button 
              ...
              in:fly={{ y: -100, duration: 400, easing: quintOut }}
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 15c-.55 0-1-.45-1-1v-4c0-.55.45-1 1-1s1 .45 1 1v4c0 .55-.45 1-1 1zm1-8h-2V7h2v2z"/></svg> 
            </button>
          </div>
        {/if}
      </div>
    ...
    <div class="w-full h-full" in:fly={{ x: -100, duration: 400, easing: quintOut }} out:fly={{ x: 100, duration: 300 }}>
      {#if $view === "my-addons"}
        <div class="config">
...
{#if communityAddonsInfo}
  ...
  >
    ...
    <SectionModal>
      <TextModal text="Community Addons are addons that are not officially supported by the OGI team." variant="body" />
      <TextModal text="They are provided as a convenience for download to the community." variant="body" />
    </SectionModal>
    <TextModal text="Please use them at your own risk. All content downloaded from addons are the responsibility of you (the user)." variant="warning" />
