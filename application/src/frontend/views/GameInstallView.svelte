<script lang="ts">
  ...
  let results: (BasicLibraryInfo & { addonsource: string })[] = $state([]);
...
{#if isOnline}
  <input
    ...
    class="p-2 pl-2 bg-slate-100 rounded-lg w-2/3 mt-4"
  />
{:else}
  <div class="flex flex-col gap-2 w-full justify-center items-center h-full">
    ...
    <h1 class="text-lg text-gray-500 text-center">
      Searching for Games is unsupported when you're offline.
    </h1>
  </div>
{/if}
...
{#if isOnline}
  <div class="games">
    ...
    {#if results.length === 0 && !loadingResults}
      <div
        class="flex justify-center text-center flex-col items-center gap-2 w-full border bg-slate-100 rounded"
      >
  ...
  {#if loadingResults}
    {#if addons.length === 0}
      <div
        class="flex justify-center text-center flex-col items-center gap-2 w-4/6 bg-slate-100 rounded p-4"
      >
        <p class="text-2xl">Hey, you have no addons!</p>
        <p class="text-gray-400 text-sm w-full">
          Addons are the core of OpenGameInstaller, and you need them to
          download, search, and setup your games! Get some online, okay?
        </p>
      </div>
    {:else}
      <div
        class="flex justify-center items-center w-1/6 border p-4 bg-slate-100 rounded"
      >
