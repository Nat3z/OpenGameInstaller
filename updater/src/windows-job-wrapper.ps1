...
public static class OgiJobRunner
{
    ...
    )
    {
        if (String.IsNullOrWhiteSpace(resultPath))
            throw new InvalidOperationException(
                "OGI_WINDOWS_JOB_RESULT is required"
            );
        ...
        if (!MoveFileEx(temporary, resultPath, 0x1 | 0x8))
            ThrowLastError("MoveFileEx result evidence");
    }
    ...
    )
    {
        ...
        try
        {
            ...
            string launchPath = Environment.GetEnvironmentVariable(
                "OGI_WINDOWS_JOB_LAUNCH"
            );
            if (!String.IsNullOrWhiteSpace(launchPath))
            {
                string launchDirectory = Path.GetDirectoryName(launchPath);
                if (!String.IsNullOrEmpty(launchDirectory))
                    Directory.CreateDirectory(launchDirectory);
                File.WriteAllText(
                    launchPath,
                    "{\"version\":1,\"rootPid\":" + process.dwProcessId +
                    ",\"killOnClose\":true}"
                );
            }

            uint waitResult = WaitTimeout;
            bool infiniteWait = timeoutMs == UInt32.MaxValue;
            DateTime waitDeadline = infiniteWait
                ? DateTime.MaxValue
                : DateTime.UtcNow.AddMilliseconds(timeoutMs);
            bool stopRequested = false;
            while (infiniteWait || DateTime.UtcNow < waitDeadline)
            {
                waitResult = WaitForSingleObject(process.hProcess, 100);
                if (waitResult == WaitFailed)
                    ThrowLastError("WaitForSingleObject");
                if (waitResult == WaitObject0) break;
                if (waitResult != WaitTimeout)
                    throw new InvalidOperationException(
                        "Unexpected process wait result: " + waitResult
                    );
                if (!String.IsNullOrWhiteSpace(stopPath) && File.Exists(stopPath))
                {
                    stopRequested = true;
                    break;
                }
            }
            bool timedOut = waitResult == WaitTimeout && !stopRequested;
            uint exitCode = 124;
...
try {
  [uint32] $timeoutMs = [uint32]::MaxValue
  if (-not [string]::IsNullOrWhiteSpace($env:OGI_WINDOWS_JOB_TIMEOUT_MS)) {
    if (-not [uint32]::TryParse($env:OGI_WINDOWS_JOB_TIMEOUT_MS, [ref] $timeoutMs) -or $timeoutMs -eq 0) {
      throw 'OGI_WINDOWS_JOB_TIMEOUT_MS must be a positive uint32 value'
    }
  }
  exit [OgiJobRunner]::Run(
    $commandLine,
    $env:OGI_WINDOWS_JOB_RESULT,
    $env:OGI_WINDOWS_JOB_STOP,
    $timeoutMs
  )
} catch {
